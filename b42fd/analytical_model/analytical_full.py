...
class Analytical_Eigenmotion:
    def __init__(self,data,motion_type,t=0, M_u=0):
        
        if motion_type =="short period motion":
            self.eigvalues, self.properties=self.get_eignval_spm(data,t, M_u) 
        
        if motion_type=="phugoid oscillation":
            self.eigvalues, self.properties=self.get_eigval_phu(data,t, M_u)
            
        if motion_type=="dutch roll":
            self.eigvalues, self.properties=self.get_eigval_dutchroll(data,t, M_u)
            
        if motion_type=="aperiodic roll":
            self.eigvalues, self.properties=self.get_eigval_ape_roll(data,t, M_u)
            
        if motion_type=="aperiodic spiral":
            self.eigvalues, self.properties=self.get_eigval_ape_spiral(data,t, M_u)
    ...
    def get_eignval_spm(self,data, t,M_u):
        params=TimeTool(data, t,M_u)
    ...
    def get_eigval_phu(self,data, t, M_u):
    ...
    def get_eigval_dutchroll(self, data, t, M_u):
    ...
    def get_eigval_ape_roll(self, data, t, M_u):
        params=TimeTool(data,t, M_u)
        mub, V0=params.mub, params.true_airspeed
    ...
    def get_eigval_ape_spiral(self, data, t, M_u):
...
if __name__ == "__main__":
    ...
    short_period= Analytical_Eigenmotion(data, "short period motion", t=t_spm,M_u=M_u)
    ...
    phugoid= Analytical_Eigenmotion(data, "phugoid oscillation", t=t_phugoid, M_u=M_u)
    ...
    dutch_roll= Analytical_Eigenmotion(data, "dutch roll", t=t_phugoid, M_u=M_u)
    ...
    aperiodic_roll= Analytical_Eigenmotion(data, "aperiodic roll", t=t_ape_roll, M_u=M_u)
    ...
    aperiodic_spiral=Analytical_Eigenmotion(data, "aperiodic spiral", t=t_ape_spiral, M_u=M_u)
    ...
    dutch_roll= Analytical_Eigenmotion(data, "dutch roll", t=t_dutchroll, M_u=M_u)
    print("eigenvalues for Dutch Roll:", dutch_roll.eigvalues)
    print("eigenvalues properties for Dutch Roll (damping ratio, period, halving time):", dutch_roll.properties)
